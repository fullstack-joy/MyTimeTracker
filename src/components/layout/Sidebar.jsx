...
export default function Sidebar() {
  ...
  return (
    <div className="w-20 lg:w-64 bg-gray-50 dark:bg-black text-gray-900 dark:text-white flex flex-col py-6 px-4 border-r border-gray-200 dark:border-gray-800">
      {/* Logo */}
      <div className="mb-10 text-center lg:text-left flex items-center justify-center lg:justify-center gap-2">
        <div className="flex items-center">
          ...
          <span className="hidden lg:inline text-gray-900 dark:text-white font-light text-xl">Tracker</span>
          {isTracking && (
            <span
      ...
      <nav className="flex-1 space-y-2">
        {menuItems.map((item, idx) => (
          <Link
            key={idx}
            to={item.to}
            className={`flex items-center space-x-4 px-3 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors border-l-4 ${
              location.pathname === item.to
                ? 'bg-gray-200 dark:bg-gray-800 text-orange-500 border-orange-500'
      ...
      <div className="mt-auto">
        <Link
          to="/help"
          className={`flex items-center space-x-4 px-3 py-2 rounded-lg hover:bg-gray-200 dark:hover:bg-gray-800 transition-colors ${
            location.pathname === '/help' ? 'bg-gray-200 dark:bg-gray-800 text-orange-500' : 'text-gray-600 dark:text-gray-300'
          }`}
        >
